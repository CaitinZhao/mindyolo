...
from .yolov7_label_assignment_np import YOLOv7LabelAssignmentNp
...
class YOLOv7LossV2(nn.Cell):
    def __init__(
        ...
        self.build_targets = YOLOv7LabelAssignmentNp(self.anchors, na=self.na, bias=0.5, stride=self.stride)
        self.one_hot = nn.OneHot(depth=self.nc, on_value=self.cp, off_value=self.cn)
        self.autobalance = False


    def cal_losses(self, p, bs, as_, gjs, gis, targets, anchors, masks):
    ...
    def construct(self, p, targets, imgs):
        bs, as_, gjs, gis, targets, anchors, masks = self.build_targets(p, targets, imgs)
        return self.cal_losses(p, bs, as_, gjs, gis, targets, anchors, masks)
...
class YOLOv7AuxLossV2(nn.Cell):
    def __init__(
        ...
        self.balance = ms.Parameter(Tensor(_balance, ms.float32), requires_grad=False)
        self.BCEcls, self.BCEobj, self.gr = BCEcls, BCEobj, 1.0
        self.build_targets = YOLOv7LabelAssignmentNp(self.anchors, na=self.na, bias=0.5,
                                                     stride=self.stride, use_aux=True)
        self.one_hot = nn.OneHot(depth=self.nc, on_value=self.cp, off_value=self.cn)
        self.autobalance = False
    ...
    def construct(self, p, targets, imgs):
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            pi_aux = p[i + self.nl]
            b, a, gj, gi, target, mask = bs[i], as_[i], gjs[i], gis[i], targets[i], masks[i]
            b_aux, a_aux, gj_aux, gi_aux, target_aux, mask_aux = \
                bs_aux[i], as_aux[i], gjs_aux[i], gis_aux[i], targets_aux[i], masks_aux[i]
            tobj = ops.zeros_like(pi[..., 0])  # target obj
...
if __name__ == '__main__':
    ...
    cfg = parse_config()
    loss_fn = create_loss(name='YOLOv7LossV2',
                          **cfg.loss,
